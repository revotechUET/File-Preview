...
mpp_service_url = os.getenv('MPP_SERVICE_URL')
uploadUrl = "{}/tasks/api/FileManager/UploadFile".format(mpp_service_url)
convertUrl = "{}/tasks/api/TasksConversion/Convert".format(mpp_service_url)
downloadUrl = "{}/tasks/api/FileManager/DownloadFile".format(mpp_service_url)
...
def ConvertFile(filename_input):
    if filename_input.lower().endswith('mpp'):
        ...
        try:
            res1 = requests.post(uploadUrl, files=files, params={
                                 'api-key': 'MY_API_KEY'})
